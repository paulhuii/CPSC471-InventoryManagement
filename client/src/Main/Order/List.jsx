...
const OrderList = () => {
         ...
         if (cartItems.length > 0 && suppliers.length > 0) {
            ...
            cartItems.forEach(ci => {
                ...
                if (existingIndex === -1) {
                    ...
                    console.log(`Item ${ci.product_name} from restock recommendations already exists for ${supplierName}. Skipping.`);
    ...
    const handleAddProductSubmit = async (newProductData) => {
        try {
            ...
            } else { throw new Error("Failed to add product, no data returned."); }
        } catch (err) {
            console.error("Failed to add product from order list:", err);
            setAddProductError(err.response?.data?.error || err.message || "Error adding product.");
        }
    };
    ...
    const handleAddSupplierSubmit = async (newSupplierData) => {
        try {
            ...
            setAddSupplierError(err.response?.data?.error || err.message || "An unexpected error occurred.");
    ...
    const addConfirmedItemToGroup = (itemData, finalSupplierName) => {
        const product = products.find(p => p.productid.toString() === itemData.productid);
         if (!product) {
         ...
         setGroupedItems(prev => {
             ...
             if (exists) {
                 alert(`${newItem.product_name} is already in the order list for ${finalSupplierName}. Edit the existing entry.`);
                 return prev; // Return previous state if item exists
             }
             // Add new item to the specific supplier's list
             return { ...prev, [finalSupplierName]: [...currentSupplierItems, newItem] };
         });
    }
    ...
    const handleAddItem = (formDataFromForm, resetFormCallback) => {
        ...
        const supplierInfo = suppliers.find(s => s.supplier_name.toLowerCase() === supplier_name.toLowerCase());
        ...
        if (supplierInfo) {
            ...
            alert(`Supplier "${supplier_name}" not found. Please add the supplier using the '+' button next to the supplier field before adding this item.`);
    ...
    const handleDeleteItem = (supplier, index) => {
        ...
        if (window.confirm(`Remove ${itemToDelete.product_name} from the order list for ${supplier}?`)) {
            setGroupedItems(prev => {
    ...
    const handlePlaceOrderForSupplier = async (supplierName, items) => {
        ...
        try {
            ...
            alert(`Order placed successfully for ${supplierName}! Order ID: ${orderid}`);
            setGroupedItems(prev => {
            ...
            alert(`Error placing order: ${error.message || 'An unknown error occurred.'}`);
        }
    };
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen">
            ...
            {Object.keys(groupedItems).length === 0 ? (
                 <p className="text-center text-gray-500 italic mt-8">Your order list is empty. Add items using the form above or from the Restock page.</p>
            ) : (
