  ...
  const handleFormSubmit = async (formData) => {
    ...
    try {
      ...
      if (isEditing) {
        const updatedData = {
          ...formData,
          [primaryKey]: currentItem[primaryKey],
        };
      ...
      setError(
        `Failed to ${isEditing ? "update" : "add"} item. ${
    ...
    if (
      window.confirm(
        `Are you sure you want to delete "${
          itemToDelete.product_name || "this item"
        }"?`
      )
    ) {
      try {
        ...
        <table className="min-w-full text-left">
          <thead>
            <tr className="text-gray-700">
              <th className="px-4 py-2" style={{ backgroundColor: "#D9BE92" }}>
                Product
              </th>
              <th className="px-4 py-2" style={{ backgroundColor: "#D9BE92" }}>
                Quantity
              ...
              return (
                <tr key={item.productid} className="border-t">
                  ...
                  <td className="px-4 py-2">
                    ${item.case_price?.toFixed(2)}/{item.order_unit}
                  </td>
                  <td className="px-4 py-2">
                    <span
                  ...
                  <td className="px-4 py-2">
                    {item.supplier?.supplier_name || "Unknown"}
                  </td>
