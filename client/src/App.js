...
const ProtectedRoute = ({ allowedRoles }) => {
    ...
    if (allowedRoles && !allowedRoles.includes(user.role?.toLowerCase())) { // Use lowercase comparison for safety
        console.warn(`ProtectedRoute: User with role '${user.role}' denied access. Redirecting.`);
             ...
             <div className="App">
                 ...
                 <main className="main-content">
                    <Routes>
                        ...
                        <Route path="/login" element={!user ? <Login /> : <Navigate to="/inventory" replace />} />
                        <Route path="/register" element={!user ? <Register /> : <Navigate to="/inventory" replace />} />

                        {/* Protected Routes (Require Login) */}
                        <Route element={<ProtectedRoute />}> {/* Base protection: must be logged in */}
                            <Route path="/inventory" element={<Inventory />} />
                            {/* Add other general protected routes here */}
                        </Route>

                        {/* Admin Only Routes */}
                        <Route element={<ProtectedRoute allowedRoles={['admin']} />}> {/* Role protection: must be admin */}
