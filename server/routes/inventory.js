...
router.get("/", authenticateToken, async (req, res) => {
  ...
  const updated = data.map((product) => {
    ...
    const latest = delivered.sort(
      (a, b) =>
        new Date(b.orders?.delivered_date) - new Date(a.orders?.delivered_date)
    )[0];
...
router.put("/:id", authenticateToken, isAdmin, async (req, res) => {
  ...
  try {
    ...
    if (!updatedData) {
      return res
        .status(404)
        .json({ error: "Product not found after update attempt." });
    }
    ...
    //
    if (
      updatedFields.hasOwnProperty("current_stock") &&
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (fetchError) {
        ...
        return res
          .status(500)
          .json({ error: "Failed to fetch product before adding stock." });
      }
      if (!currentProduct) {
        return res.status(404).json({ error: "Product not found." });
      }
      ...
      if (!updatedData) {
        return res
          .status(404)
          .json({ error: "Product not found after stock update attempt." });
      }
      ...
      if (transactionError) {
        console.error(
          "Failed to log inventory transaction for stock add:",
          transactionError
        );
      }
      ...
      console.error(
        "Unexpected error in POST /inventory/:productid/add-stock:",
        err
      );
      res.status(500).json({ error: "An unexpected server error occurred." });
    }
