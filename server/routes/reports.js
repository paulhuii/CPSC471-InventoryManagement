...
router.get("/summary", authenticateToken, async (req, res) => {
  try {
    ...
    // Get active products currently in stock
    ...
    if (activeProducts && activeProducts.length > 0) {
        ...
        // Get the delivered purchase prices for these products 
        ...
        //Create a map of product ID to its latest cost (using the sorted array)
        const latestCosts = {};
        // Iterate the SORTED array now
        sortedPriceDetails.forEach(detail => {
            //  sorted by date descending, the first price for a product is the latest
        ...
        // Calculate total value using the latest cost found
...
// --- GET /api/reports/monthly-top-products ---
// Fetches top N products ordered within a specific month and year
router.get("/monthly-top-products", authenticateToken,  async (req, res) => {
    ...
    try {
        ...
        (monthlyOrderDetails || []).forEach(detail => {
            if (!detail.products || !detail.productid) return; 
